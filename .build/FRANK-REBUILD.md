...
```
/projects/frank              Frank's repo — this is what you edit
/projects/blockwise          another project — DO NOT EDIT
/projects/<other>            other projects — DO NOT EDIT
/frank/deployed/infra        docker compose + Caddy config for the running stack
~/agent-skills/skills        the shared skill library (markdown, read by agents)
/home/codex/.hermes          Hermes itself — DO NOT EDIT
```
...
1. **Never edit anything under `/home/codex/.hermes`.** That is Hermes. It is
   updated by its own tool.
2. **Never edit `/projects/blockwise` or any project other than `/projects/frank`**
...
```bash
git add .build/STATE.md
git commit -m "<TASK-ID>: claim"
```

Then create your branch:

```bash
git checkout -b codex/<TASK-ID>
```
...
1. Run the verification commands listed in the task. All must pass.
2. Set your row in `.build/STATE.md` to `DONE`, with the commit hash.
3. Commit and push. **Do not merge.**

### If you get stuck

Write what happened into `.build/tasks/<TASK-ID>.md` under a heading
`## BLOCKED`, set your row to `BLOCKED`, commit, push, stop.
...
`.build/tasks/<TASK-ID>.md` under `## HOT-FILE REQUEST`, mark yourself
`BLOCKED`, and stop.
...
If a task adds database changes, also:
...
3. Add a **new** migration (do not edit an existing one) that renames each table
   to `legacy_brain_*`. Do not drop them.
...
```
anything named mission_*, workbench_*, worktree_*
```
...
editing `frank-shell.tsx`, that is a hot file — write a `## HOT-FILE REQUEST`
into your task file with the exact lines to remove, mark yourself `BLOCKED`,
and stop.
...
**Forbidden:** `.env` files, anything under `/home/codex/.hermes`
...
A read-only browser for `/projects`. The owner cannot use a terminal.
...
- resolves the path
- **rejects anything that resolves outside `/projects`** — test this
...
**Done when:**
- [ ] You can browse `/projects` in a browser and read a file
- [ ] `GET /v1/files?path=/etc/passwd` returns 403
- [ ] `GET /v1/files?path=/projects/../etc/passwd` returns 403
- [ ] `GET /v1/files?path=/projects/frank/.env` returns 403
...
**Forbidden:** writing to `~/agent-skills`

Lists every skill in `~/agent-skills/skills`. For each: its name, its
description (from the frontmatter at the top of `SKILL.md`), and the rendered
markdown.
...
**Depends on:** Wave 4
**Allowed:** `~/agent-skills/skills/ad-template-design/**`
**Forbidden:** any code

**No code in this task.** Write markdown.

Create `~/agent-skills/skills/ad-template-design/SKILL.md` with frontmatter
`name` and `description`, then sections covering: what makes a good ad
template, the exact required sizes (Feed 1080×1350, Story 1080×1920), safe
zones, text limits, what gets rejected, and how Story differs from Feed —
**Story is a redesign, never a crop or a stretch of Feed.**
...
## Appendix D — Traps already found in this codebase

These cost real time before. Do not rediscover them.

**Drizzle transactions look like the pool.** A `PgTransaction` inherits
`.transaction()` from `PgDatabase` — that is how savepoints open. So
`typeof db.transaction === 'function'` is **true for both** a pool handle and an
already-open transaction. Using it to decide "do I need to open a transaction?"
makes the function recurse forever, one savepoint per level, until the
connection dies. Postgres shows `idle in transaction` on `savepoint spNNNNN`
and nothing logs an error.

`rollback()` exists **only** on `PgTransaction`. That is the discriminator:

```ts
const isOpenTransaction = typeof (db as { rollback?: unknown }).rollback === 'function';
```

This matters for W4-2, which writes steps and pieces inside transactions.

**Floating promises hide every failure.** The old code dispatched work with
`void doThing().catch(log)`. When `doThing()` *hung* rather than threw, nothing
logged and the job sat in `queued` forever. Every background call in the runner
must have a timeout and must write a terminal row. A job that can sit in
`queued` with no explanation is a bug even if it never happens.

**A single-file bind mount in Docker goes stale.** Editing a file that is
bind-mounted into a container as a single file does not change what the
container sees — the inode is different. Mount the *directory*, or restart the
container. `docker compose reload` will report success and change nothing.

**Never put a `build:` block on a digest-pinned service.** `docker compose up`
will rebuild it and silently replace a released image with a local build.
