...
# Recovery must never inherit a live sender/scheduler activation.
jq -e '.mute_emails == 1 and .enable_scheduler == 0' "$restored_config" >/dev/null || fail "restored site is not safely muted"
if [[ "${OWNER_CRM_VERIFY_EMAIL_ACCOUNT:-0}" == 1 ]]; then
