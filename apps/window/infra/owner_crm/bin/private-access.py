...
SERVICES={"8445":"http://127.0.0.1:18081","8446":"http://127.0.0.1:18104","8447":"http://127.0.0.1:18106"}
def state():
...
def validate_routes(config, host):
    for port, target in SERVICES.items():
        key = host + ":" + port
        if key in config.get("Web", {}) and config["Web"][key] != {"Handlers": {"/": {"Proxy": target}}}:
            raise RuntimeError("Private access port already belongs to another service")
        if port in config.get("TCP", {}) and config["TCP"][port] != {"HTTPS": True}:
            raise RuntimeError("Private access TCP port is occupied")
        if config.get("AllowFunnel", {}).get(key):
            raise RuntimeError("Refusing a public Funnel port")
...
def main():
    ...
    print(json.dumps({"mode":"applied" if args.apply else "preview","crm_url":"https://"+host+":8445","notifications_url":"https://"+host+":8446","marketing_url":"https://"+host+":8447","tailnet_only":True,"native_app_auth_required":True,"phone_receipt_verified":False}))
