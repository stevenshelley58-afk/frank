...
[[ -f "$HELPER_RECEIPT" && ! -L "$HELPER_RECEIPT" ]] || die "helper checksum receipt is unavailable"
...
approved_helper_sha="$(tr -d '\n' <"$HELPER_RECEIPT")"
[[ "$helper_sha" =~ ^[0-9a-f]{64}$ && "$approved_helper_sha" == "$helper_sha" ]] || die "helper checksum is not approved"
