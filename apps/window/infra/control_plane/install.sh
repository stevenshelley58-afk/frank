...
if [[ -n "${FRANK_EXPECTED_REVISION:-}" ]]; then
  ...
  git -C /projects/frank diff --quiet "$expected" -- apps/window/infra/control_plane apps/window/infra/checkout apps/window/infra/cleanup apps/window/infra/discovery apps/window/infra/evaluations apps/window/infra/retention apps/window/scripts apps/window/graph governance/control-plane || { echo "canonical control-plane hook differs from expected immutable revision" >&2; exit 1; }
  [[ -z "$(git -C /projects/frank ls-files --others --exclude-standard -- apps/window/infra/control_plane apps/window/infra/checkout apps/window/infra/cleanup apps/window/infra/discovery apps/window/infra/evaluations apps/window/infra/retention apps/window/scripts apps/window/graph governance/control-plane)" ]] || { echo "canonical control-plane hook closure has untracked inputs" >&2; exit 1; }
fi
