...
[[ "$(stat -c "%U:%a" "$secret_dir")" == root:700 ]] || die "secret directory must be root-owned mode 0700"
...
grep -Eq '^auth-default-access:[[:space:]]*deny-all[[:space:]]*$' "$config_file" || die "config auth default is not deny-all"
grep -Eq '^cache-duration:[[:space:]]+[0-9]+(h|m|s)$' "$config_file" || die "config cache duration is not bounded"
...
printf '%s' "$cfg" | python3 -c 'import json,sys; s=json.load(sys.stdin)["services"]["ntfy"]; p=s["ports"]; assert len(p)==1 and p[0]["host_ip"] in ("127.0.0.1","::1"); assert set(s["networks"])=={"owner_notifications_private","owner_crm_webhook"}; assert not s.get("env_file")' || die "compose policy failed"
docker inspect frank-owner-ntfy >/dev/null 2>&1 || die "container missing"
[[ "$(docker inspect --format "{{index .Config.Labels \"org.frank.source-sha\"}}" frank-owner-ntfy)" == "$source_sha" ]] || die "runtime source SHA does not match HEAD"
[[ "$(docker inspect --format "{{index .Config.Labels \"org.frank.image-digest\"}}" frank-owner-ntfy)" == "sha256:416507e2100728cac39bb911fd26759a1de872ca8d1e428d08db73054b33dc85" ]] || die "runtime image digest label mismatch"
[[ "$(docker inspect --format '{{.State.Health.Status}}' frank-owner-ntfy)" == healthy ]] || die "container unhealthy"
net="$(docker inspect --format '{{json .NetworkSettings.Networks}}' frank-owner-ntfy)"; printf '%s' "$net" | python3 -c 'import json,sys; assert set(json.load(sys.stdin))=={"frank_owner_notifications_private","owner-crm_owner-crm-internal"}' || die "network policy failed"
[[ "$(docker network inspect --format '{{.Internal}}' owner-crm_owner-crm-internal)" == true ]] || die "owner CRM Webhook network must remain internal"
