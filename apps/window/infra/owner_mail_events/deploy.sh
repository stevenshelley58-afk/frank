...
for f in apps/window/owner_mail_events.py apps/window/owner_mail_reply.py "$script_dir/owner_mail_events_app.py" "$script_dir/requirements.txt" "$script_dir/owner-mail-events.service"; do [[ -f "$f" ]] || die "missing $f"; done
