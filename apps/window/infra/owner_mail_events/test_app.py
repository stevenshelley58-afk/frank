...
class EventError(Exception):
 def __init__(self,message="rejected",safe_code="provider_recipient_rejected"): super().__init__(message); self.safe_code=safe_code
...
def load(process):
 receiver=types.SimpleNamespace(SAFE_ERROR_CODES=frozenset({"provider_recipient_rejected","callback_recipient_rejected","recipient_mismatch","native_write_unavailable","processing_rejected"}),OwnerMailEventError=EventError,OwnerMailEventUnavailable=Unavailable,OwnerMailEventsConfig=Config,process_event=process)
...
class Tests(unittest.TestCase):
 ...
 def test_invalid_and_transient_log_only_allowlisted_codes(self):
  for code in ("provider_recipient_rejected","callback_recipient_rejected","recipient_mismatch"):
   with self.subTest(code=code):
    app=load(lambda *args, safe_code=code: (_ for _ in ()).throw(EventError("payload-secret-marker",safe_code)))
    ...
    self.assertIn("code="+code+" status=400",logged)
