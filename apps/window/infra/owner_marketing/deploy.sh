...
cd "$script_dir/../../../.."; [[ -z "$(git status --porcelain)" ]] || die dirty-or-untracked-source; git ls-files --error-unmatch apps/window/infra/owner_marketing/compose.yml apps/window/infra/owner_marketing/nginx.conf apps/window/infra/owner_marketing/deploy.sh apps/window/infra/owner_marketing/install.sh apps/window/infra/owner_marketing/check.sh >/dev/null || die untracked-source; export MAUTIC_SOURCE_SHA="$(git rev-parse HEAD)"; cd "$script_dir"
