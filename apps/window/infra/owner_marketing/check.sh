...
docker exec -w /var/www/html frank-owner-marketing php bin/console debug:config framework mailer --no-interaction | grep -Fq "%env(urlencoded-dsn:MAUTIC_MAILER_DSN)%" || die native-mailer-dsn-unconfigured
...
if [[ "$mailer_dsn" == "smtp://mail-sink.invalid:25" ]]; then
  ...
  docker exec frank-owner-marketing php -r '$u=parse_url(getenv("MAUTIC_MAILER_DSN")); if (($u["scheme"] ?? "") !== "smtp" || empty($u["host"])) exit(2); $port=(int)($u["port"] ?? 587); $s=@fsockopen($u["host"], $port, $e, $s, 5); if (!$s) exit(1); fclose($s);' || die smtp-egress-unreachable
fi
counts="$(docker exec frank-owner-marketing-db sh -lc 'mysql -uroot -p"$MYSQL_ROOT_PASSWORD" -Nse "SELECT (SELECT COUNT(*) FROM leads), (SELECT COUNT(*) FROM campaigns) FROM DUAL" mautic')"; [[ "$counts" == $'0\t0' ]] || die nonempty-contacts-or-campaigns
echo "healthy: native Mautic admin login through loopback ingress, protected routes deny unauthenticated access, empty contacts/campaigns, native runtime profile policy, mail transport policy, private backend network"
