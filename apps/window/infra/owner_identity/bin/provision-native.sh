...
for key in OWNER_IDENTITY_BOOTSTRAP_TOKEN OWNER_IDENTITY_HOST OWNER_CRM_ORIGIN OWNER_CRM_CONTAINER OWNER_CRM_SITE OWNER_MARKETING_CONTAINER; do [[ -n $(value "$key") ]] || die "$key must be set in $secret_file"; done
...
frappe_json=$(docker exec -e OWNER_IDENTITY_BOOTSTRAP_TOKEN="$token" "$identity_container" python3 - <<'PY'
import json, os
...
docker exec -i -e MAUTIC_SAML_SP_ENTITY_ID="${OWNER_IDENTITY_MAUTIC_SP_ENTITY_ID:-https://mail.blockwise.sale}" -e MAUTIC_SAML_IDP_METADATA="$metadata" "$marketing_container" php -r '
