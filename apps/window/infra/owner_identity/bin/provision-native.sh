...
docker cp "$root_dir/native/frappe_owner_entry/." "$crm_container:/home/frappe/frappe-bench/apps/frank_owner_entry"
docker exec "$crm_container" bash -lc "chown -R frappe:frappe /home/frappe/frappe-bench/apps/frank_owner_entry && cd /home/frappe/frappe-bench && ./env/bin/pip install -e apps/frank_owner_entry >/dev/null && (bench --site '$crm_site' list-apps | grep -qx frank_owner_entry || bench --site '$crm_site' install-app frank_owner_entry)" || die "Frappe native entry installation failed"
...
metadata=$(curl --fail --silent --show-error --proto '=https' --max-time 15 "$metadata_url") || die "could not fetch Mautic IdP metadata"
...
docker cp "$root_dir/native/mautic_frank_owner_entry" "$marketing_container:/var/www/html/docroot/plugins/FrankOwnerEntryBundle"
docker exec "$marketing_container" chown -R www-data:www-data /var/www/html/docroot/plugins/FrankOwnerEntryBundle || die "Mautic native return installation failed"
