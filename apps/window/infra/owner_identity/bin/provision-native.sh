...
# LightSAML derives Mautic's only ACS from its SP entity ID and validates the
# Response Destination and bearer Recipient against it, so the entity ID and
# Authentik's ACS must both be the native marketing origin.
[[ "$mautic_entity" == "$marketing_origin" ]] || die "OWNER_IDENTITY_MAUTIC_SP_ENTITY_ID must be the native marketing origin"
...
frappe_json=$(docker exec -i -e OWNER_IDENTITY_BOOTSTRAP_TOKEN="$token" -e MAUTIC_SAML_ACS_URL="$mautic_acs" -e MAUTIC_SAML_SP_ENTITY_ID="$mautic_entity" "$identity_container" python3 - <<'PY'
import json, os
...
expected_audience=os.environ["MAUTIC_SAML_SP_ENTITY_ID"]
patch={}
if saml[0].get("sp_binding") != "post": patch["sp_binding"]="post"
if saml[0].get("acs_url") != expected_acs: patch["acs_url"]=expected_acs
if saml[0].get("audience") != expected_audience: patch["audience"]=expected_audience
if patch:
    ...
    if saml.get("sp_binding") != "post" or saml.get("acs_url") != expected_acs or saml.get("audience") != expected_audience: raise SystemExit("Mautic SAML native ACS, audience or POST binding was not applied")
...
# Mautic loads the native account by the username claim. Authentik's
# ws/2005 "name" claim is the display name, so the username claim must be
# Authentik's own saml/username, which carries the mapped native login name.
