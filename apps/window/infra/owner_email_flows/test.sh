...
python3 -m py_compile "$s/mautic_flows.py" "$s/test_mautic_flows.py" "$s/source_adapter.py" "$s/source_operate.py" "$s/source_scheduled.py" "$s/activate_source_adapter.py" "$s/provision_integration_identity.py" "$s/test_integration_identity.py"
python3 "$s/test_mautic_flows.py"
python3 "$s/test_source_adapter.py"
python3 "$s/test_integration_identity.py"
