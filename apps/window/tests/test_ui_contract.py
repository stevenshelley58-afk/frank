...
class UiContractTest(unittest.TestCase):
    ...
    def test_ad_studio_is_a_real_tool_surface_backed_by_hermes(self):
        ...
        self.assertIn('aria-label="Ad Studio"', html)
        ...
        self.assertIn('A generated reciprocal reference checks the source measurement', html)
        self.assertIn('Up to four focused passes and two final patch passes', html)
        self.assertIn('Two final reviewers score the finished result once.', html)
        self.assertIn('manual Ready for Review queue', html)
        ...
        self.assertIn('Each image becomes its own background job', html)
        ...
        self.assertIn('Number(data.score) >= 9.8 ? "accepted" : "revise"', studio)
        self.assertNotIn('Number(data.score) >= 9.5 ? "accepted" : "revise"', studio)
        ...
        self.assertIn('setAdStudioActive(id === \"ad-studio\")', app)
        ...
        self.assertIn('mergeAdStudioRunList(runs, incoming)', studio)
        ...
        self.assertIn('listAdStudioRuns({ projectId, limit: 100 })', studio)
        ...
        self.assertIn('/app.css?v=20260905-blockwise-operations-preview-v1', html)
        self.assertIn('/js/app.js?v=20260905-blockwise-operations-preview-v1', html)
        self.assertIn('./ad-studio.js?v=20260905-ready-review-v1', app)
        self.assertIn('groupAdStudioRuns(runs)', studio)
        ...
        self.assertIn('fetch(`/api/ad-studio/models', studio)
        self.assertIn('Only for these Ad Studio jobs', html)
        ...
        self.assertNotIn('/api/ad-studio/runs/" + encodeURIComponent(run.id) + "/download', studio)
        ...
        self.assertRegex(app, r'project_id: projectId,\s+name: jobName,\s+brief,\s+attachments:')
    ...
    def test_ad_studio_batches_click_and_drop_source_images(self):
        html = (WEB / "index.html").read_text(encoding="utf-8")
        app = (WEB / "js" / "app.js").read_text(encoding="utf-8")
        studio = (WEB / "js" / "ad-studio.js").read_text(encoding="utf-8")
        styles = (WEB / "app.css").read_text(encoding="utf-8")
        ...
        for state in ('queued: "Ready"', 'uploading: "Uploading…"', 'starting: "Starting…"', 'started: "Job started"', 'error: "Needs attention"'):
