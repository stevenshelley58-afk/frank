...
class TemplateReleaseRouteTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        (cls.release_root / "release-1" / "pack.json").write_text(
            '{"schema":"blockwise.template-pack/v2"}', encoding="utf-8"
        )
    ...
    def test_pack_sample_and_font_are_readable(self):
        ...
        for artifact, content_type in expected:
            response = self.client.get(
                f"/releases/ad-template-generator/release-1/{artifact}"
            )
            try:
                self.assertEqual(response.status_code, 200)
                self.assertEqual(
                    response.data,
                    (self.release_root / "release-1" / artifact).read_bytes(),
                )
    ...
    def test_conditional_get_and_head_preserve_immutable_contract(self):
    ...
    def test_symlink_is_not_served(self):
        ...
        self.assertEqual(
            self.client.get(
                "/releases/ad-template-generator/release-1/linked.json"
            ).status_code,
            404,
        )
