...
class FailureTest(LibraryTestBase):
    def test_health_checks_writable_candidate_queue_not_approved_root(self):
        with patch("shared_library.os.access", side_effect=lambda path, mode: Path(path) == self.root / "candidates"):
            health = self.library.health()
        self.assertEqual(health["status"], "ready")
        self.assertTrue(health["candidate_queue_writable"])
