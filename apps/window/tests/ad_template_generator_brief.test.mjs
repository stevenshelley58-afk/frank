...
import { AD_TEMPLATE_GENERATOR_BRIEF_MAX_CHARACTERS, adTemplateGeneratorBriefLength, adTemplateGeneratorBriefValidation } from "../web/js/ad-template-generator-brief.js";
...
test("a 1,975-character rich brief is accepted without normalization", () => {
  ...
  const validation = adTemplateGeneratorBriefValidation(brief);

  assert.equal(AD_TEMPLATE_GENERATOR_BRIEF_MAX_CHARACTERS, 4000);
  assert.equal(adTemplateGeneratorBriefLength(brief), 1975);
...
test("4,001 characters are rejected and never returned as a shortened value", () => {
  ...
  assert.equal(adTemplateGeneratorBriefLength(brief), 4001);
...
test("4,000 multibyte characters are accepted at the boundary", () => {
  ...
  assert.equal(adTemplateGeneratorBriefLength(brief), 4000);
  assert.equal(adTemplateGeneratorBriefValidation(brief).valid, true);
});
