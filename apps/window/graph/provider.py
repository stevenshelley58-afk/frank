...
The blueprint is intentionally not registered with Frank's production app.
Final assembly supplies authorized reader callbacks after the accepted main
runtime is frozen.
"""
...
_QUERY_KEYS = frozenset({"lens", "settings_revision_id"})
...
def _query() -> dict[str, str | None]:
    ...
    result = {"lens": lens}
    settings_revision_id = request.args.get("settings_revision_id")
    if settings_revision_id is not None and not _OPAQUE.fullmatch(settings_revision_id):
        abort(400, "invalid opaque graph selector")
    result["settings_revision_id"] = settings_revision_id
    return result
...
def _safe_graph(value: object, *, kind: str, entity_id: str, selectors: Mapping[str, str | None]) -> dict:
    ...
    requested_revision = selectors.get("settings_revision_id")
    if requested_revision is not None and requested_revision not in settings_revisions:
        raise ProviderUnavailable("graph settings revision did not match the requested selector")
