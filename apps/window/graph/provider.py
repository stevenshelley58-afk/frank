...
V2_GRAPH_SCHEMA = "schema://frank.graph/v2"
KNOWLEDGE_LENS = "knowledge.combined"
MAX_V2_NODES = 5_000
MAX_V2_EDGES = 10_000
MAX_V2_BODY_BYTES = 8 * 1024 * 1024
# Public names make the hard boundary easy for integration tests and callers
# to inspect without reaching into the v1 implementation details.
MAX_GRAPH_NODES = MAX_V2_NODES
MAX_GRAPH_EDGES = MAX_V2_EDGES
MAX_GRAPH_BODY_BYTES = MAX_V2_BODY_BYTES
_V2_PROJECT_ID = re.compile(r"^[a-z0-9][a-z0-9._-]{0,63}$")
_V2_SOURCE_REF = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._:/-]{0,639}$")
...
def _v2_source(value: object, label: str) -> dict:
    source = _exact(
        value,
        {"provider_id", "provider_version", "source_type", "source_ref", "sha256"},
        label,
    )
    _safe_text(source["provider_id"], f"{label}.provider_id", pattern=_OPAQUE, max_length=160)
    _safe_text(source["provider_version"], f"{label}.provider_version", pattern=_SAFE_REFERENCE, max_length=160)
    _safe_text(source["source_type"], f"{label}.source_type", pattern=_OPAQUE, max_length=160)
    _safe_text(source["source_ref"], f"{label}.source_ref", pattern=_V2_SOURCE_REF, max_length=640)
    if not isinstance(source["sha256"], str) or not SHA256.fullmatch(source["sha256"]):
        raise ProviderUnavailable(f"{label}.sha256 was invalid")
    return source


def _v2_bounded_list(value: object, label: str, *, limit: int) -> list:
    if not isinstance(value, list) or len(value) > limit:
        raise ProviderUnavailable(f"{label} exceeded its hard limit")
    return value


def _safe_v2_metadata(value: object, label: str) -> object:
    """Validate metadata without dropping fields or interpreting provider data."""
    if value is None:
        return None
    if not isinstance(value, (dict, list)):
        raise ProviderUnavailable(f"{label} must be JSON metadata")
    # _safe_json has the established JSON/markup/secret checks.  A separate
    # sensitive-key walk makes this v2 boundary reject rather than redact.
    try:
        if _contains_sensitive_key(value, label):
            raise ProviderUnavailable(f"{label} contained secret-like or sensitive data")
        return _safe_json(value, label)
    except ProviderUnavailable:
        raise
    except (RecursionError, TypeError, ValueError) as error:
        raise ProviderUnavailable(f"{label} was not safe JSON metadata") from error


def validate_knowledge_graph(value: object, *, project_id: str) -> dict:
    """Validate a Hermes-owned generic project knowledge graph (schema v2).

    This is deliberately separate from ``_safe_graph``: the v1 validator is a
    stable Tool-manifest adapter and its pipeline-specific source identities and
    lenses must not be widened to accommodate knowledge projections.
    """
    if not isinstance(project_id, str) or not _V2_PROJECT_ID.fullmatch(project_id):
        raise ProviderUnavailable("knowledge project identity was invalid")
    if not isinstance(value, dict):
        raise ProviderUnavailable("knowledge graph was not an object")
    try:
        # Check the serialized shape before walking untrusted nested data.  This
        # is also the body-size guard for dependency-injected readers.
        encoded = json.dumps(value, ensure_ascii=False, separators=(",", ":"), allow_nan=False).encode("utf-8")
    except (TypeError, ValueError, OverflowError, RecursionError) as error:
        raise ProviderUnavailable("knowledge graph was not bounded JSON") from error
    if len(encoded) > MAX_V2_BODY_BYTES:
        raise ProviderUnavailable("knowledge graph exceeded its body limit")
    ...
    graph = _exact(value, required, "knowledge graph envelope")
    if graph["schema"] != V2_GRAPH_SCHEMA:
        raise ProviderUnavailable("knowledge graph had an unsupported schema")
    graph_id = graph["graph_id"]
    if not isinstance(graph_id, str) or not _GRAPH_ID.fullmatch(graph_id) or len(graph_id) > 320:
        raise ProviderUnavailable("knowledge graph identity was invalid")
    if not isinstance(graph["graph_revision"], str) or not SHA256.fullmatch(graph["graph_revision"]):
        raise ProviderUnavailable("knowledge graph revision was invalid")
    _timestamp(graph["generated_at"], "knowledge.graph.generated_at")
    provider = _exact(graph["provider"], {"id", "version", "authority"}, "knowledge.graph.provider")
    _safe_text(provider["id"], "knowledge.graph.provider.id", pattern=_OPAQUE, max_length=160)
    if not isinstance(provider["version"], str) or not SEMVER.fullmatch(provider["version"]):
        raise ProviderUnavailable("knowledge graph provider version was invalid")
    if provider["authority"] not in _AUTHORITIES:
        raise ProviderUnavailable("knowledge graph provider authority was invalid")
    subject = _exact(graph["subject"], {"kind", "id"}, "knowledge.graph.subject")
    scope = _exact(graph["scope"], {"kind", "id"}, "knowledge.graph.scope")
    if subject != {"kind": "project", "id": project_id} or scope != subject:
        raise ProviderUnavailable("knowledge graph subject or scope did not match the project")
    if graph["lens"] != KNOWLEDGE_LENS:
        raise ProviderUnavailable("knowledge graph lens was not knowledge.combined")
    capabilities = graph["capabilities"]
    if not isinstance(capabilities, list) or len(capabilities) > 128 or len(capabilities) != len(set(capabilities)):
        raise ProviderUnavailable("knowledge graph capabilities were invalid")
    for item in capabilities:
        _safe_text(item, "knowledge.graph.capabilities[]", pattern=_OPAQUE, max_length=160)
    if graph["trace_ref"] is not None:
        raise ProviderUnavailable("knowledge graphs must not contain trace references")
    _extensions(graph["extensions"], "knowledge.graph.extensions", allowed=set())

    nodes = _v2_bounded_list(graph["nodes"], "knowledge.graph.nodes", limit=MAX_V2_NODES)
    edges = _v2_bounded_list(graph["edges"], "knowledge.graph.edges", limit=MAX_V2_EDGES)
    groups = _v2_bounded_list(graph["groups"], "knowledge.graph.groups", limit=MAX_V2_NODES)
    node_ids: set[str] = set()
    node_required = {
        "id", "source_id", "kind", "label", "scope", "authority", "source",
        "classification", "freshness", "capabilities", "ports", "status",
        "presentation", "extensions", "metadata",
    }
    for index, node in enumerate(nodes):
        node = _exact(node, node_required, f"knowledge.graph.nodes[{index}]", optional={"metadata"})
        node_id = node["id"]
        if not isinstance(node_id, str) or not _GRAPH_ID.fullmatch(node_id) or not node_id.startswith(graph_id + "/") or node_id in node_ids:
            raise ProviderUnavailable("knowledge node identity was invalid or duplicated")
        node_ids.add(node_id)
        _safe_text(node["source_id"], f"knowledge.graph.nodes[{index}].source_id", pattern=_OPAQUE, max_length=160)
        _safe_text(node["kind"], f"knowledge.graph.nodes[{index}].kind", pattern=_OPAQUE, max_length=160)
        _safe_text(node["label"], f"knowledge.graph.nodes[{index}].label", max_length=1024)
        if node["scope"] != scope or node["authority"] not in _AUTHORITIES or node["classification"] not in _CLASSIFICATIONS or node["status"] not in NODE_STATUSES:
            raise ProviderUnavailable("knowledge node provenance or state was invalid")
        _freshness(node["freshness"], f"knowledge.graph.nodes[{index}].freshness")
        _v2_source(node["source"], f"knowledge.graph.nodes[{index}].source")
        if not isinstance(node["capabilities"], list) or len(node["capabilities"]) > 128 or len(node["capabilities"]) != len(set(node["capabilities"])):
            raise ProviderUnavailable("knowledge node capabilities were invalid")
        for capability in node["capabilities"]:
            _safe_text(capability, f"knowledge.graph.nodes[{index}].capabilities[]", pattern=_OPAQUE, max_length=160)
        if node["ports"] != []:
            raise ProviderUnavailable("knowledge node ports must be empty")
        _node_presentation(node["presentation"], f"knowledge.graph.nodes[{index}].presentation", generic=True)
        _extensions(node["extensions"], f"knowledge.graph.nodes[{index}].extensions", allowed=set())
        if "metadata" in node:
            _safe_v2_metadata(node["metadata"], f"knowledge.graph.nodes[{index}].metadata")

    edge_ids: set[str] = set()
    edge_required = {
        "id", "source_id", "from", "to", "kind", "authority", "classification",
        "source", "freshness", "status", "presentation", "extensions", "metadata",
    }
    for index, edge in enumerate(edges):
        edge = _exact(edge, edge_required, f"knowledge.graph.edges[{index}]", optional={"metadata"})
        edge_id = edge["id"]
        if not isinstance(edge_id, str) or not _GRAPH_ID.fullmatch(edge_id) or not edge_id.startswith(graph_id + "/") or edge_id in edge_ids:
            raise ProviderUnavailable("knowledge edge identity was invalid or duplicated")
        edge_ids.add(edge_id)
        if not isinstance(edge["source_id"], (str, int)) or isinstance(edge["source_id"], bool):
            raise ProviderUnavailable("knowledge edge source identity was invalid")
        if isinstance(edge["source_id"], str):
            _safe_text(edge["source_id"], f"knowledge.graph.edges[{index}].source_id", pattern=_OPAQUE, max_length=160)
        elif not 0 <= edge["source_id"] <= _MAX_SAFE_INTEGER:
            raise ProviderUnavailable("knowledge edge source identity was invalid")
        if edge["from"] not in node_ids or edge["to"] not in node_ids:
            raise ProviderUnavailable("knowledge edge referenced an unknown node")
        if edge["kind"] not in {"relation", "association", "dependency", "reference", "control"} or edge["authority"] not in _AUTHORITIES or edge["classification"] not in _CLASSIFICATIONS or edge["status"] not in EDGE_STATUSES:
            raise ProviderUnavailable("knowledge edge provenance or state was invalid")
        _v2_source(edge["source"], f"knowledge.graph.edges[{index}].source")
        _freshness(edge["freshness"], f"knowledge.graph.edges[{index}].freshness")
        _edge_presentation(edge["presentation"], f"knowledge.graph.edges[{index}].presentation")
        _extensions(edge["extensions"], f"knowledge.graph.edges[{index}].extensions", allowed=set())
        if "metadata" in edge:
            _safe_v2_metadata(edge["metadata"], f"knowledge.graph.edges[{index}].metadata")

    group_ids: set[str] = set()
    group_parents: dict[str, str | None] = {}
    for index, group in enumerate(groups):
        group = _exact(group, {"id", "label", "parent_id", "order", "metadata"}, f"knowledge.graph.groups[{index}]", optional={"metadata"})
        group_id = group["id"]
        if not isinstance(group_id, str) or not _GRAPH_ID.fullmatch(group_id) or not group_id.startswith(graph_id + "/") or group_id in group_ids:
            raise ProviderUnavailable("knowledge group identity was invalid or duplicated")
        group_ids.add(group_id)
        _safe_text(group["label"], f"knowledge.graph.groups[{index}].label", max_length=1024)
        parent = group["parent_id"]
        if parent is not None and (not isinstance(parent, str) or not _GRAPH_ID.fullmatch(parent) or not parent.startswith(graph_id + "/")):
            raise ProviderUnavailable("knowledge group parent was invalid")
        group_parents[group_id] = parent
        if not isinstance(group["order"], int) or isinstance(group["order"], bool) or not 0 <= group["order"] <= _MAX_SAFE_INTEGER:
            raise ProviderUnavailable("knowledge group order was invalid")
        if "metadata" in group:
            _safe_v2_metadata(group["metadata"], f"knowledge.graph.groups[{index}].metadata")
    for group_id, parent in group_parents.items():
        if parent is not None and parent not in group_ids:
            raise ProviderUnavailable("knowledge group parent was unknown")
    visiting: set[str] = set()
    visited: set[str] = set()
    def visit(group_id: str) -> None:
        if group_id in visiting:
            raise ProviderUnavailable("knowledge groups must be acyclic")
        if group_id in visited:
            return
        visiting.add(group_id)
        parent = group_parents[group_id]
    ...
    for node in nodes:
        ...
        if group_id is not None and (not isinstance(group_id, str) or group_id not in group_ids):
            raise ProviderUnavailable("knowledge node referenced an unknown group")
    for edge in edges:
        group_id = edge["presentation"].get("group_id")
        if group_id is not None and (not isinstance(group_id, str) or group_id not in group_ids):
            raise ProviderUnavailable("knowledge edge referenced an unknown group")
    try:
        if _contains_sensitive_key(graph):
            raise ProviderUnavailable("knowledge graph contained unsafe fields")
    except ProviderUnavailable:
        raise
    except (RecursionError, TypeError, ValueError) as error:
        raise ProviderUnavailable("knowledge graph contained unsafe fields") from error
    return deepcopy(graph)


class _NoRedirect(urllib.request.HTTPRedirectHandler):
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        raise ProviderUnavailable("knowledge projection redirects are not allowed")


class KnowledgeProjectionClient:
    """Small, fail-closed HTTP client for the Hermes-owned projection."""

    def __init__(self, url: str, token: str, project_ids: object, *, timeout: float = 2.5, opener=None):
        if not isinstance(url, str) or not url or len(url) > 2048:
            raise ValueError("projection URL is required")
        ...
        if parsed.scheme not in {"http", "https"} or not parsed.netloc or parsed.query or parsed.fragment or parsed.username or parsed.password:
            raise ValueError("projection URL must be an absolute HTTP(S) URL without credentials or query")
        if not isinstance(token, str) or not token or any(ord(char) <= 32 for char in token) or len(token) > 4096:
            raise ValueError("projection token is required")
        if isinstance(project_ids, str):
            project_ids = [item.strip() for item in project_ids.split(",") if item.strip()]
        if not isinstance(project_ids, (list, tuple, set, frozenset)):
            raise ValueError("allowed project IDs are required")
        normalized_ids = []
        for item in project_ids:
            if isinstance(item, str) and item.startswith("project/"):
                item = item[len("project/"):]
            normalized_ids.append(item)
        if any(not isinstance(item, str) for item in normalized_ids):
            raise ValueError("allowed project IDs were invalid")
        if len(normalized_ids) != len(set(normalized_ids)):
            raise ValueError("allowed project IDs must not contain duplicates")
        ids = tuple(sorted(normalized_ids))
        if not ids or any(not isinstance(item, str) or not _V2_PROJECT_ID.fullmatch(item) for item in ids):
            raise ValueError("allowed project IDs were invalid")
        self.url = url.rstrip("/")
        self.token = token
        self.project_ids = frozenset(ids)
    ...
    @classmethod
    def from_environment(cls) -> "KnowledgeProjectionClient | None":
        url = os.environ.get("FRANK_KNOWLEDGE_PROJECTION_URL", "").strip()
        token = os.environ.get("FRANK_KNOWLEDGE_PROJECTION_TOKEN", "")
        ids = os.environ.get("FRANK_KNOWLEDGE_ALLOWED_PROJECTS", "")
        if not url or not token or not ids.strip():
            return None
        try:
            return cls(url, token, ids)
        except (TypeError, ValueError):
            return None

    def configured_for(self, project_id: str) -> bool:
        return project_id in self.project_ids

    def fetch(self, project_id: str) -> dict:
        if not self.configured_for(project_id):
            raise ProviderUnavailable("knowledge projection is not configured for this project")
        # The configured URL is the dedicated, read-only projection endpoint.
        # Keep the project identity in the query so the Frank bridge never
        # guesses or rewrites a provider route (and so a future projection
        # gateway can remain independent from Frank's public graph API).
        query = urllib.parse.urlencode({"project": f"project/{project_id}", "lens": KNOWLEDGE_LENS})
        url = f"{self.url}?{query}"
        req = urllib.request.Request(url, headers={"Accept": "application/json", "Authorization": f"Bearer {self.token}"}, method="GET")
        try:
            ...
            try:
                ...
                if length and (not length.isdigit() or int(length) > MAX_V2_BODY_BYTES):
                    raise ProviderUnavailable("knowledge projection response exceeded its body limit")
                if status != 200:
                    raise ProviderUnavailable("knowledge projection was unavailable")
                body = response.read(MAX_V2_BODY_BYTES + 1)
            finally:
                response.close()
        except ProviderUnavailable:
            raise
        except (urllib.error.URLError, urllib.error.HTTPError, OSError, ValueError, TimeoutError) as error:
            raise ProviderUnavailable("knowledge projection was unavailable") from error
        if len(body) > MAX_V2_BODY_BYTES:
            raise ProviderUnavailable("knowledge projection response exceeded its body limit")
        try:
            payload = json.loads(body.decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError) as error:
            raise ProviderUnavailable("knowledge projection was not valid JSON") from error
        try:
            return validate_knowledge_graph(payload, project_id=project_id)
        except ProviderUnavailable:
            raise
        except Exception as error:
            # Untrusted recursive/oversized structures must become a normal
            # unavailable response, never a server error or traceback.
            raise ProviderUnavailable("knowledge projection was invalid") from error


class KnowledgeProjectionProvider:
    def __init__(self, client: KnowledgeProjectionClient | None):
        self.client = client

    def graph(self, *, kind: str, entity_id: str, selectors: Mapping[str, str | None]) -> dict:
        if kind != "project" or selectors.get("lens") != KNOWLEDGE_LENS or self.client is None:
            raise ProviderUnavailable("knowledge projection is unavailable")
        return self.client.fetch(entity_id)
...
def create_blueprint(provider: ReadOnlyProvider, knowledge_provider: KnowledgeProjectionProvider | None = None) -> Blueprint:
    ...
    @api.get("/api/graphs/project/<entity_id>")
    def knowledge_graph_read(entity_id: str):
        # Knowledge is a separate Hermes-owned projection path.  It never
        # falls back to the v1 Tool manifest reader or any local data source.
        if knowledge_provider is None:
            return _unavailable(ProviderUnavailable("knowledge projection is unavailable"))
        if not _V2_PROJECT_ID.fullmatch(entity_id):
            abort(404)
        unknown = set(request.args) - {"lens"}
        if unknown or request.args.get("lens") != KNOWLEDGE_LENS:
            abort(400, "unsupported knowledge graph selector")
        try:
            payload = knowledge_provider.graph(
                kind="project", entity_id=entity_id, selectors={"lens": KNOWLEDGE_LENS}
            )
        except ProviderUnavailable as error:
            return _unavailable(error)
        return jsonify(payload), 200, {"Cache-Control": "no-store"}
