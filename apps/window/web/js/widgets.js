...
function mountLaunchDesk(el) {
  el.replaceChildren();
  const actions = toolIntro(el, "Blockwise owner work lives in one workspace. Open the workspace to use the native mail, CRM, support, campaign, revenue, results, and notification surfaces.");
  actionButton(actions, "Open Blockwise workspace", () => emit("frank:project-home", "blockwise"));
}
