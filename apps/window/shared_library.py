...
def _default_root() -> Path:
    ...
    if Path("/data").is_dir():
        return Path("/data/knowledge/shared-library")
...
class CentralLibrary:
    ...
    def health(self) -> dict[str, Any]:
        try:
            ...
            # Approved data is intentionally root-owned/read-only. Candidate
            # contributions use the dedicated queue when it exists; retain a
            # safe fallback for older installs that have only the root.
            writable_path = self.candidates if self.candidates.exists() else self.root
            writable = (writable_path.is_dir() and not writable_path.is_symlink()
                        and os.access(writable_path, os.W_OK | os.X_OK))
