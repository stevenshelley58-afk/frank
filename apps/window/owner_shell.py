...
def resolve_spa_document(web_root: Path, path: str) -> tuple[Path, str]:
    ...
    bundle is built, and everything else gets the vanilla Window index. A path
    that escapes ``web_root``, by traversal or by an absolute segment, never
    names a file here: it falls through to the vanilla index, and the caller is
    free to reject it earlier with its own error.
    """
    ...
    if owner_shell_route(requested):
