...
export class S3ObjectStorage implements ObjectStorage {
  constructor(private readonly endpoint: URL, private readonly allowedHosts: readonly string[], private readonly client: { head(bucket: string, key: string, signal?: AbortSignal): Promise<{ size: bigint } | undefined>; read(bucket: string, key: string, range?: { start: bigint; end?: bigint }, signal?: AbortSignal): Promise<Readable>; copy(source: { bucket: string; key: string }, target: { bucket: string; key: string }, signal?: AbortSignal): Promise<void>; remove(bucket: string, key: string, signal?: AbortSignal): Promise<void> }) {
