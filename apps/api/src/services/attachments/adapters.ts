...
export class HmacUploadCapability implements UploadCapabilityPort {
  ...
  async verify(value: string): Promise<UploadCapabilityClaims | undefined> { const parts = value.split('.'); if (parts.length !== 3 || parts[0] !== 'v1' || !this.matches(parts[1]!, parts[2]!)) return undefined; try { const p = JSON.parse(Buffer.from(parts[1]!, 'base64url').toString('utf8')) as Record<string, unknown>; if (typeof p.u !== 'string' || typeof p.c !== 'string' || typeof p.o !== 'string' || typeof p.v !== 'number' || typeof p.e !== 'number' || !Number.isSafeInteger(p.v) || !Number.isSafeInteger(p.e) || p.e <= this.now().getTime()) return undefined; return { uploadId: p.u, cellId: p.c, ownerId: p.o, capabilityVersion: p.v, expiresAt: new Date(p.e) }; } catch { return undefined; } }
