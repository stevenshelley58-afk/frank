...
export class InternalTusdTerminator implements TusdTerminationPort {
  ...
  constructor(baseUrl: string, private readonly request: typeof fetch = fetch) { const base = new URL(baseUrl); if (base.protocol !== 'http:' || base.hostname !== 'frank-tusd' || base.username || base.password || base.search || base.hash) throw new Error('invalid_internal_tusd_endpoint'); this.endpoint = base; }
  async terminate(input: { uploadId: string; capability: string }): Promise<void> { if (!/^[0-9a-f-]{36}$/i.test(input.uploadId)) throw new Error('invalid_upload_id'); const url = new URL(`v1/uploads/tus/${input.uploadId}`, this.endpoint.pathname.endsWith('/') ? this.endpoint : new URL(`${this.endpoint.pathname}/`, this.endpoint)); const response = await this.request(url, { method: 'DELETE', headers: { 'Tus-Resumable': '1.0.0', 'X-Frank-Upload-Capability': input.capability }, redirect: 'error' }); if (response.status !== 204 && response.status !== 404) throw new Error(`tusd_terminate_${response.status}`); }
}
