...
export class AttachmentLifecycle {
  ...
  async authorizeTusGate(input: GateInput): Promise<LifecycleResult<UploadReservation>> {
    ...
    const internalTermination = claims.purpose === 'termination' && input.method === 'DELETE' && reservation?.state === 'terminating';
    if (!reservation || reservation.ownerId !== claims.ownerId || reservation.cellId !== claims.cellId || claims.capabilityVersion > reservation.capabilityVersion + (internalTermination ? 1 : 0) || claims.capabilityVersion < reservation.capabilityVersion - 1 || (!internalTermination && reservation.expiresAt <= this.now()) || (!internalTermination && !['authorized', 'uploading'].includes(reservation.state))) return this.error('forbidden', 'Upload is not resumable.', 'never');
  ...
  async cancel(uploadId: string, ownerId: string, cellId: string, capability: string, tusd: TusdTerminationPort): Promise<'accepted' | 'already_terminated'> { const reservation = await this.requestCancel(uploadId, ownerId, cellId); const authorized = await this.authorizeTusGate({ uploadId, capability, method: 'DELETE', cellId }); if (authorized.kind === 'error') throw new AttachmentCancelError('forbidden'); const begun = await this.persistence.beginTermination({ cellId, uploadId: reservation.uploadId, ownerId }); if (begun === 'not_found') throw new AttachmentCancelError('not_found'); if (begun === 'already_terminated') return begun; try { const outcome = await tusd.terminate({ uploadId: reservation.uploadId, capability }); if (outcome === 'absent') await this.persistence.persistTermination({ cellId, uploadId: reservation.uploadId, reason: 'cancelled' }); } catch (error) { await this.persistence.retryTermination({ cellId, uploadId: reservation.uploadId, reason: error instanceof Error ? error.message : 'tusd_termination_failed' }); throw new AttachmentCancelError('unavailable'); } return 'accepted'; }
  /** Called only by the server expiry loop after a 0013 cleanup-ledger claim. */
  async expire(reservation: UploadReservation, tusd: TusdTerminationPort): Promise<void> { if (reservation.state !== 'terminating' || reservation.expiresAt > this.now()) throw new Error('invalid_expiry_claim'); const capability = await this.capabilities.issue({ uploadId: reservation.uploadId, cellId: reservation.cellId, ownerId: reservation.ownerId, capabilityVersion: reservation.capabilityVersion + 1, expiresAt: new Date(this.now().getTime() + MAX_CAPABILITY_TTL_MS), purpose: 'termination' }); try { const outcome = await tusd.terminate({ uploadId: reservation.uploadId, capability }); if (outcome === 'absent') await this.persistence.persistTermination({ cellId: reservation.cellId, uploadId: reservation.uploadId, reason: 'expired' }); } catch (error) { await this.persistence.retryTermination({ cellId: reservation.cellId, uploadId: reservation.uploadId, reason: error instanceof Error ? error.message : 'expiry_tusd_termination_failed' }); throw error; } }
