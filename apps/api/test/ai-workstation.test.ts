...
describe("AI workstation API routes", () => {
  ...
  it("allows the Frank repo workspace for lab AI sessions with an older workspace allowlist", async () => {
    const pool = new FakeAiPool();
    const fetchMock = vi.fn(async () => jsonResponse({
      id: "host-session-2",
      sessionName: "frank-codex-host-session-2",
      tool: "codex",
      workspacePath: "/opt/frank-hub",
      status: "running",
      createdAt: "2026-05-05T00:00:00.000Z",
    ...
    const response = await server.inject({
      ...
      payload: {
        tool: "codex",
        workspacePath: "/opt/frank-hub",
        prompt: "continue Frank"
      }
    });

    expect(response.statusCode).toBe(201);
    expect(fetchMock).toHaveBeenCalledWith(
      "http://host-agent.local/v1/sessions",
      expect.objectContaining({ method: "POST" })
    );
  });

  it("describes blocked AI workspaces with the blocked path and configured allowlist", async () => {
    const { server } = createTestServer(new FakeAiPool(), {
      operator: {
        allowedWorkspaces: ["/opt/frank-hub/workspaces"]
      }
    ...
    expect(response.json().message).toContain("/srv/not-frank");
    expect(response.json().message).toContain("/opt/frank-hub/workspaces");
    expect(response.json().message).toContain("/opt/frank-hub");
  });

  it("preserves safe host-agent start errors for AI sessions", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => jsonResponse({
...
function createTestServer(pool: FakeAiPool, overrides: TestConfigOverrides = {}) {
