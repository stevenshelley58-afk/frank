...
script_dir="$(cd "$(dirname "$0")" && pwd)"; harness_dir="$(cd "$script_dir/.." && pwd)"; repo_root="${FRANK_EVIDENCE_REPO_ROOT:-$(git -C "$harness_dir" rev-parse --show-toplevel)}"
...
const digest = /^sha256:[a-f0-9]{64}$/; const safeUrl = value => /^https:\/\/(?![^/]*\.invalid(?:\/|$))[^\s]+$/.test(value || ''); const privateReceiptUri = value => /^frank-evidence:\/\/vps\/srv\/frank\/evidence\/[a-z0-9-]+\/[A-Za-z0-9._-]+$/.test(value || '');
...
for (const [service, [tag, license, config]] of Object.entries(expected)) {
  ...
  if (provenance.status === 'approved-exception') { const exception=provenance.approved_exception; if (!exception || !/^(docs\/evidence\/|frank-evidence:\/\/)/.test(exception.record_uri || '') || !digest.test(exception.record_sha256) || !exception.reason?.trim() || !exception.reviewed_by?.trim() || !/^\d{4}-\d\d-\d\dT/.test(exception.reviewed_at || '')) fail(`unapproved provenance exception for ${service}`); } else if (provenance.status !== 'verified' || provenance.approved_exception) fail(`invalid provenance status for ${service}`);
  ...
  if (phase === 'sealed-promotion' && (!(safeUrl(item.hosted_canary_url) || privateReceiptUri(item.hosted_canary_url)) || !item.server_command?.trim())) fail(`sealed canary evidence missing for ${service}`);
}
NODE
