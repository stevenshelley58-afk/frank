...
## Independent release review, 14 September 2026

The supplied a0702c3 handoff was reviewed before production cutover. Its
production-baseline claim was stale: the actual running Window image was
fbebd0499da9b402c405eb0cd4262dd3c856fc73, not 238f5c9.

Focused checks on the supplied candidate passed: 191 owner/API/infra Python
tests, 35 webmail tests, 38 native configuration tests, and 37 owner JavaScript
tests. These do not prove a successful fresh production deployment.

Independent review found two packaging defects despite those passes:
- The Dockerfile did not copy the newly imported owner Python modules. A fresh
  image would fail server startup. The release now copies owner_*.py.
- Caddy's Compose networks omitted identity and CRM ingress; manual attachments
  would disappear on recreation. Both networks are now declared, and a focused
  infrastructure regression test covers these release requirements.

An additional owner-only access-policy gap on the CRM and Mautic proxy
