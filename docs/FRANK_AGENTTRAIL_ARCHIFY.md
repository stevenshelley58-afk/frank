...
The pinned AgentTrail source is recorded as a submodule at `5b97cf3cef548a0c668731e7f569fa36c14832f2`. Run one loopback observer per implementation repository, for example:

```bash
node apps/window/vendor/agenttrail/bin/agenttrail.mjs /projects/only-process-frank --port 5340 --no-open
node apps/window/vendor/agenttrail/bin/agenttrail.mjs /projects/only-process-hermes --port 5341 --no-open
node apps/window/vendor/agenttrail/bin/agenttrail.mjs /projects/only-process-blockwise --port 5342 --no-open
```
