...
- Real signed bounce and complaint callback acceptance passed on 14 September 2026. The two stored provider events were replayed through Resend's own webhook replay API, not a new send: the receiver returned 204 with zero rejections, the complaint contact stopped its nurture, took native email Do Not Contact and left every source segment, and the invalid-signature and mismatched-recipient controls were rejected without mutation. The fault was Resend rewriting its own simulator mailbox (`complained+...@resend.dev` to `complaint+...@simulator.amazonses.com`, `bounced+...@resend.dev` to `bounce+...@simulator.amazonses.com`) against a byte-exact recipient comparison; revision `3034623` admits only that exact, closed alias for the callback leg and leaves the signature, provider-recipient and immutable-identity guards unchanged. Receipt: `/srv/frank/verification/owner-mail-events-callback-fix-20260914/receipt.json`. Separately, the receiver's Mautic lookups had been failing because `trusted_proxies` held a stale ingress container address; re-running `configure_site.sh` restored the loopback API, and the recurrence risk is an open owner-marketing decision. Native durable follow-up and the controlled inbox-reply stop are now implemented and verified as described above. Exact marketing-thread correlation remains unproven.
