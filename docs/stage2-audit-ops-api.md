...
Collectors are strictly allowlisted and best-effort. Docker and cloudflared
return unavailable fields when the runtime cannot safely access the needed
status. Deploy metadata is read from `runtime/deploy.json`, generated by
`scripts/deploy.sh`, and returns unavailable cleanly when the file is missing or
invalid. Ops routes do not accept command input and do not return raw
