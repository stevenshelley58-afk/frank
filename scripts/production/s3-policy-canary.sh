...
if [[ -n "${FRANK_LAKE_BUCKET:-}" && -n "${FRANK_LAKE_WORKER_ACCESS_KEY:-}" && -n "${FRANK_LAKE_WORKER_SECRET_KEY:-}" && -n "${FRANK_LAKE_QUERY_ACCESS_KEY:-}" && -n "${FRANK_LAKE_QUERY_SECRET_KEY:-}" ]]; then
  ...
  if aws_for "$FRANK_STAGING_ACCESS_KEY" "$FRANK_STAGING_SECRET_KEY" head-object --bucket "$FRANK_LAKE_BUCKET" --key "$lake_key" >/dev/null 2>&1; then echo 'staging lake overreach' >&2; exit 1; fi
  for pair in "$FRANK_PROMOTER_ACCESS_KEY:$FRANK_PROMOTER_SECRET_KEY" "$FRANK_DOWNLOADER_ACCESS_KEY:$FRANK_DOWNLOADER_SECRET_KEY"; do IFS=: read -r access secret <<< "$pair"; if aws_for "$access" "$secret" head-object --bucket "$FRANK_LAKE_BUCKET" --key "$lake_key" >/dev/null 2>&1; then echo 'attachment lake overreach' >&2; exit 1; fi; done
  for pair in "$FRANK_LAKE_WORKER_ACCESS_KEY:$FRANK_LAKE_WORKER_SECRET_KEY" "$FRANK_LAKE_QUERY_ACCESS_KEY:$FRANK_LAKE_QUERY_SECRET_KEY"; do IFS=: read -r access secret <<< "$pair"; for spec in "frank-attachment-staging:$key" "frank-objects:$object" "frank-object-previews:$preview"; do b="${spec%%:*}"; k="${spec#*:}"; if aws_for "$access" "$secret" head-object --bucket "$b" --key "$k" >/dev/null 2>&1 || aws_for "$access" "$secret" list-objects-v2 --bucket "$b" >/dev/null 2>&1 || aws_for "$access" "$secret" put-object --bucket "$b" --key "$k" --body /dev/null 2>&1; then echo 'lake attachment overreach' >&2; exit 1; fi; done; done
