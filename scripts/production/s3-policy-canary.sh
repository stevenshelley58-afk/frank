...
readback="$(mktemp)"; trap 'rm -f -- "$readback"' EXIT
...
aws_for "$FRANK_PROMOTER_ACCESS_KEY" "$FRANK_PROMOTER_SECRET_KEY" get-object --bucket frank-objects --key "$object" "$readback" >/dev/null
...
aws_for "$FRANK_PROMOTER_ACCESS_KEY" "$FRANK_PROMOTER_SECRET_KEY" delete-object --bucket frank-attachment-staging --key "$key"
if aws_for "$FRANK_PROMOTER_ACCESS_KEY" "$FRANK_PROMOTER_SECRET_KEY" head-object --bucket frank-attachment-staging --key "$key" >/dev/null 2>&1; then echo 'promoter staging cleanup failed' >&2; exit 1; fi
...
if aws_for "$FRANK_DOWNLOADER_ACCESS_KEY" "$FRANK_DOWNLOADER_SECRET_KEY" delete-object --bucket frank-objects --key "$object" >/dev/null 2>&1; then echo 'downloader delete overreach' >&2; exit 1; fi
