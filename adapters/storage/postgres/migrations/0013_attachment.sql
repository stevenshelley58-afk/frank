...
CREATE TABLE "frank_domain"."upload_reservation" (
  "id" uuid PRIMARY KEY NOT NULL, "cell_id" text NOT NULL, "owner_id" text NOT NULL, "conversation_id" uuid NOT NULL, "idempotency_key" text NOT NULL,
  ...
  CONSTRAINT "upload_reservation_idempotency_uidx" UNIQUE("cell_id","owner_id","conversation_id","idempotency_key"), CONSTRAINT "upload_reservation_upload_uidx" UNIQUE("cell_id","upload_id"),
  ...
  CONSTRAINT "upload_reservation_expiry_strict" CHECK ("expires_at" > "created_at" AND "expires_at" <= "created_at" + interval '15 minutes'),
...
CREATE TABLE "frank_domain"."attachment_message_quota" (
  "cell_id" text NOT NULL, "message_id" uuid NOT NULL, "reserved_bytes" bigint NOT NULL DEFAULT 0 CHECK ("reserved_bytes" >= 0 AND "reserved_bytes" <= 10737418240), "reserved_count" integer NOT NULL DEFAULT 0 CHECK ("reserved_count" >= 0 AND "reserved_count" <= 10000), "updated_at" timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY("cell_id","message_id"), CONSTRAINT "attachment_message_quota_message_cell_fk" FOREIGN KEY("message_id","cell_id") REFERENCES "frank_domain"."chat_message"("id","cell_id") ON DELETE CASCADE, CONSTRAINT "attachment_message_quota_cell_not_blank" CHECK (length(btrim("cell_id")) > 0)
);--> statement-breakpoint
...
CREATE TABLE "frank_domain"."attachment" (
  "id" uuid PRIMARY KEY NOT NULL, "cell_id" text NOT NULL, "owner_id" text NOT NULL, "conversation_id" uuid, "message_id" uuid, "turn_id" uuid, "reservation_id" uuid NOT NULL, "upload_id" text NOT NULL, "object_id" uuid,
  ...
  CONSTRAINT "attachment_source_ref_object" CHECK (jsonb_typeof("source_ref") = 'object' AND "source_ref" ? 'kind' AND "source_ref" ? 'id' AND length(btrim("source_ref"->>'kind')) > 0 AND length(btrim("source_ref"->>'id')) > 0 AND (NOT ("source_ref" ? 'version') OR length(btrim("source_ref"->>'version')) > 0)),
...
CREATE TABLE "frank_domain"."attachment_cleanup_ledger" (
  ...
  CONSTRAINT "attachment_cleanup_reservation_cell_fk" FOREIGN KEY("reservation_id","cell_id") REFERENCES "frank_domain"."upload_reservation"("id","cell_id") ON DELETE CASCADE, CONSTRAINT "attachment_cleanup_once_uidx" UNIQUE("cell_id","reservation_id"), CONSTRAINT "attachment_cleanup_state_consistent" CHECK (("state" = 'claimed') = ("claimed_at" IS NOT NULL) AND (("state" = 'completed') = ("completed_at" IS NOT NULL)), CONSTRAINT "attachment_cleanup_cell_not_blank" CHECK (length(btrim("cell_id")) > 0)
);--> statement-breakpoint
