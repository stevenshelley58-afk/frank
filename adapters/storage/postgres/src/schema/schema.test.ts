...
describe('Wave 1 frozen contracts', () => {
  ...
  it('freezes the 10k attachment cardinality and the exact normative schema bytes', () => {
    ...
    for (const [file, expected] of Object.entries(hashes)) {
      // Hash LF-normalized bytes so CRLF checkouts (Windows) cannot break the
      // frozen contract: the freeze was computed on the repo blob (LF).
      const content = readFileSync(path.join(CONTRACT_SCHEMAS_DIR, file), 'utf8').replace(/\r\n/g, '\n');
